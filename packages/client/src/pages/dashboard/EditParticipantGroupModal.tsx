...
import { useUpdateParticipantGroup } from '../../api/participants';
...
const EditParticipantGroupModal = ({ setIsOpen, isOpen, group }: Props) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const { mutate: updateGroup, isLoading: isUpdatingGroup } = useUpdateParticipantGroup(group?.id);
  ...
  const isUpdated =
    newName !== group?.name || JSON.stringify(participants) !== JSON.stringify(group?.participants);

  const handleUpdateGroup = async ({ name }: FieldValues) => {
    const data = {
      name,
      participants
    };
  ...
  const handleBulkFormSubmit = async () => {
    ...
    const filteredEmails = parsedEmails.filter(
      (email) => !group?.participants.find((p) => p.email === email.email)
    );
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-default transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <form className="mt-8" onSubmit={handleSubmit(handleUpdateGroup)}>
                  <div className="mb-4">
                    <InputWithLabel
                      label="Group name"
                  ...
                  <div className="flex justify-end mt-8">
                    <ActionButton
                      ...
                      disabled={!isUpdated || isUpdatingGroup}
                    >
                      Update Group
                    </ActionButton>
                  </div>
...
export default EditParticipantGroupModal;
